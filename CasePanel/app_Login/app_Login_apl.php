...
if (!isset($this->sc_temp_usr_login)) {$this->sc_temp_usr_login = (isset($_SESSION['usr_login'])) ? $_SESSION['usr_login'] : "";}
	...
	$sql = "INSERT INTO
				sec_usuariosusers( login, name, pswd, email, active )
			VALUES 
				(". $this->Db->qstr($usr_id). ", ".
					$this->Db->qstr($name) . ", ".
	...
	$sql = "INSERT INTO
				sec_usuariosusers_social( sec_usuariosusers_social.login, sec_usuariosusers_social.resource, sec_usuariosusers_social.resource_id )
			VALUES 
				(". $this->Db->qstr($usr_id). ", ".
	...
	$sql = "INSERT INTO
				sec_usuariosusers_groups( login, group_id )
			VALUES 
				(". $this->Db->qstr($usr_id). ", ".
...
$sql = "SELECT 
		...
		priv_export,
		priv_print
	      FROM sec_usuariosgroups_apps
	      WHERE group_id IN
	          (SELECT
		       group_id
		   FROM
		       sec_usuariosusers_groups 
		   WHERE
...
;
	...
	if($this->sc_logged($this->login )):
        ...
        if (is_file($_SESSION['scriptcase']['dir_temp'] . "/sc_apl_default_Web_Case_IoT_V_1_12.txt")) {
    unlink($_SESSION['scriptcase']['dir_temp'] . "/sc_apl_default_Web_Case_IoT_V_1_12.txt");
}
if (is_file("../_lib/friendly_url/app_Login_ini.txt")) {
    $SC_arq_def = fopen($_SESSION['scriptcase']['dir_temp'] . "/sc_apl_default_Web_Case_IoT_V_1_12.txt", "w");
    fwrite ($SC_arq_def, 'app_Login, R');
    fclose ($SC_arq_def);
    setcookie('sc_apl_default_Web_Case_IoT_V_1_12','app_Login, R','0','/');
}
;
		 if (isset($this->sc_temp_usr_login)) { $_SESSION['usr_login'] = $this->sc_temp_usr_login;}
 if (!isset($this->Campos_Mens_erro) || empty($this->Campos_Mens_erro))
 {
$this->nmgp_redireciona_form($this->Ini->path_link . "" . SC_dir_app_name('menu_cima') . "/", $this->nm_location, "", "_self", "ret_self", 440, 630);
 };	
	endif;
}
if (isset($this->sc_temp_usr_login)) { $_SESSION['usr_login'] = $this->sc_temp_usr_login;}
