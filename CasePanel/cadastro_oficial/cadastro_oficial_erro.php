...
$old_error_handler = set_error_handler("cadastro_oficial_scriptcase_error_handler");

function cadastro_oficial_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['cadastro_oficial']['contr_erro']) && $_SESSION['scriptcase']['cadastro_oficial']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_cadastro_oficial->contr_cadastro_oficial->Apl_com_erro = true;
            if ($inicial_cadastro_oficial->contr_cadastro_oficial->NM_ajax_flag)
            {
                $inicial_cadastro_oficial->contr_cadastro_oficial->NM_ajax_info['errList']['geral_cadastro_oficial'][] = $msg;
                return;
            }
            cadastro_oficial_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function cadastro_oficial_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['cadastro_oficial'])  && '' != $_SESSION['scriptcase']['error_icon']['cadastro_oficial'])  ? $_SESSION['scriptcase']['error_icon']['cadastro_oficial']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['cadastro_oficial']) && '' != $_SESSION['scriptcase']['error_close']['cadastro_oficial']) ? $_SESSION['scriptcase']['error_close']['cadastro_oficial'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['cadastro_oficial']) && '' != $_SESSION['scriptcase']['error_span_title']['cadastro_oficial']) ? $_SESSION['scriptcase']['error_span_title']['cadastro_oficial'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['cadastro_oficial']) && '' != $_SESSION['scriptcase']['error_icon_title']['cadastro_oficial']) ? $_SESSION['scriptcase']['error_icon_title']['cadastro_oficial'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['cadastro_oficial']['error_buffer'] .= ob_get_flush();
}
