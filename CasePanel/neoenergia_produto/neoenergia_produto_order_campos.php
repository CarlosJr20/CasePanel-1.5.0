...
<?php
   ...
   $Ord_Cmp = new neoenergia_produto_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class neoenergia_produto_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $use_alias, $tab_ger_campos, $tab_def_campos, $tab_def_seq, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['neoenergia_produto']['ordem_select_orig']))
   {
       $_SESSION['sc_session'][$sc_init]['neoenergia_produto']['ordem_select_orig'] = array();
   }
   ...
   $tab_def_seq[20] = "data";
   $tab_labels["data"]   = (isset($_SESSION['sc_session'][$sc_init]['neoenergia_produto']['labels']["data"])) ? $_SESSION['sc_session'][$sc_init]['neoenergia_produto']['labels']["data"] : "Data";
   $tab_ger_campos['hora'] = "on";
   $tab_def_campos['hora'] = "hora";
   $tab_def_seq[30] = "hora";
   $tab_labels["hora"]   = (isset($_SESSION['sc_session'][$sc_init]['neoenergia_produto']['labels']["hora"])) ? $_SESSION['sc_session'][$sc_init]['neoenergia_produto']['labels']["hora"] : "Hora";
   $tab_ger_campos['produto'] = "on";
   $tab_def_campos['produto'] = "produto";
   $tab_def_seq[40] = "produto";
   $tab_labels["produto"]   = (isset($_SESSION['sc_session'][$sc_init]['neoenergia_produto']['labels']["produto"])) ? $_SESSION['sc_session'][$sc_init]['neoenergia_produto']['labels']["produto"] : "Produto";
   $tab_ger_campos['codigo'] = "on";
   $tab_def_campos['codigo'] = "codigo";
   $tab_def_seq[50] = "codigo";
   $tab_labels["codigo"]   = (isset($_SESSION['sc_session'][$sc_init]['neoenergia_produto']['labels']["codigo"])) ? $_SESSION['sc_session'][$sc_init]['neoenergia_produto']['labels']["codigo"] : "Codigo";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['neoenergia_produto']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['neoenergia_produto']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['neoenergia_produto']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['neoenergia_produto']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['neoenergia_produto']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['neoenergia_produto']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['neoenergia_produto']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['neoenergia_produto']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       if (isset($tab_def_campos[$campo]))
       {
           ...
           if (isset($_SESSION['sc_session'][$sc_init]['neoenergia_produto']['ordem_quebra']))
           {
               foreach($_SESSION['sc_session'][$sc_init]['neoenergia_produto']['ordem_quebra'] as $campo_quebra => $resto) 
               {
                   foreach($resto as $sqldef => $ordem_ant) 
                   {
                       if ($sqldef == $tab_def_campos[$campo]) 
                       { 
                           $_SESSION['sc_session'][$sc_init]['neoenergia_produto']['ordem_quebra'][$campo][$sqldef] = $ordem;
           ...
           if (!$Ordem_tem_quebra)
           {
               $_SESSION['sc_session'][$sc_init]['neoenergia_produto']['ordem_select'][$tab_def_campos[$campo]] = $ordem;
           }   
       }
   }
?>
...
function Sel_processa_out_regr($sel_regra)
{
   ...
   foreach ($regras_ord as $cada_regra)
   {
       ...
       {
           $_SESSION['sc_session'][$sc_init]['neoenergia_produto']['ordem_select'][$tab_def_seq[$seq]] = $ordem;
       }
   }
?>
    <script language="javascript"> 
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "neoenergia_produto_order_campos.php",
    data: {
   ...
   <table class="<?php echo ($embbed)? '':'scGridTabela'; ?>" style="border-width: 0; border-collapse: collapse; width:100%;" cellspacing=0 cellpadding=0>
    ...
    <td colspan=5>
     <select  name="sel_regra" id="id_sel_regra_sel_ord" size=1 class="scAppDivToolbarInput">
       <OPTION value=""></OPTION>
       <OPTION value="-;10;id#_fld_#-;20;data#_fld_#-;30;hora#_fld_#-;40;produto#_fld_#-;50;codigo">Mais Recente</OPTION>
    </select>
<?php
   if (!$embbed)
   {
...
<?php
   ...
   $arr_order = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['neoenergia_produto']['ordem_select_orig'] : $_SESSION['sc_session'][$sc_init]['neoenergia_produto']['ordem_select'];
...
<?php
  $disp_rest = "none";
  if (isset($_SESSION['sc_session'][$sc_init]['neoenergia_produto']['ordem_select']) && $_SESSION['sc_session'][$sc_init]['neoenergia_produto']['ordem_select'] != $_SESSION['sc_session'][$sc_init]['neoenergia_produto']['ordem_select_orig']) {
      $disp_rest = "";
  }
   if (!$embbed)
   {
