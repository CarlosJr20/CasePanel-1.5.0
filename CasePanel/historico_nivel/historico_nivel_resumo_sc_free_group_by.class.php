...
class historico_nivel_resumo
{
   ...
   function initializeButtons()
   {
      ...
      $this->nmgp_botoes['chart_exit'] = isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['dashboard_info']['under_dashboard'] ? "off" : "on";

      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['dashboard_info']['under_dashboard'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['dashboard_info']['maximized']) {
          $tmpDashboardApp = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['dashboard_info']['dashboard_app'];
          if (isset($_SESSION['scriptcase']['dashboard_toolbar'][$tmpDashboardApp]['historico_nivel'])) {
              $tmpDashboardButtons = $_SESSION['scriptcase']['dashboard_toolbar'][$tmpDashboardApp]['historico_nivel'];
      ...
      if (isset($_SESSION['scriptcase']['sc_apl_conf']['historico_nivel']['btn_display']) && !empty($_SESSION['scriptcase']['sc_apl_conf']['historico_nivel']['btn_display']))
      {
          foreach ($_SESSION['scriptcase']['sc_apl_conf']['historico_nivel']['btn_display'] as $NM_cada_btn => $NM_cada_opc)
          {
   ...
   { 
	...
	function generateChartImages() {
		...
		$chartList  = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_charts'];
		$chartFiles = array();

		$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['phantomjs_export_process'] = true;
		$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['export_progress_step']     = 'image';
		$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['export_progress_count']    = 0;
		$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['export_progress_total']    = 0;
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['export_progress_total']++;
		}
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['phantomjs_export_file'] = '';
			$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['export_progress_count']++;

			$this->writeProgressFile();

			$this->Graf->generateChartImage($chartKey);

			if ('' != $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['phantomjs_export_file']) {
				$chartFiles[] = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['phantomjs_export_file'];
			}
		}

		$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['phantomjs_export_process'] = false;
	...
	function zipChartImages($password = '') {
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['export_progress_step'] = 'zip';

		$this->writeProgressFile();

		$zipFile = $this->zipFileList($chartImages, $password);

		$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['export_progress_step'] = 'end';
	...
	function writeProgressFile() {
		if ('image' == $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_pdff_pcht'] . ": {$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['export_progress_count']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['export_progress_total']}###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('zip' == $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_chrt_zip_img'] . "###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('end' == $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['export_progress_step']) {
			$content = "ok###100";
		}
		else {
			$content = "init###0";
		...
		$f = @fopen("{$this->Ini->root}{$_SESSION['scriptcase']['historico_nivel']['glo_nm_path_imag_temp']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['export_progress_file']}", 'w');
		fwrite($f, $content);
		fclose($f);
	}
   ...
   function monta_resumo($b_export = false)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search']))
      { 
         $this->res_contr = new historico_nivel_apl();
      ...
      $this->sc_where_orig   = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['where_orig'];
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['where_pesq'];
      $this->sc_where_filtro = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['where_pesq_filtro'];
     if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['campos_busca']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['campos_busca']))
     { 
         $Busca_temp = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['campos_busca'];
         ...
         {
             ob_start();
             $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['chart_combination_type'] = $_POST['fc_chart_type'];
         }

       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_y_axys']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_y_axys']))
       {
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_x_axys']             = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_y_axys']             = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['summarizing_fields_order'] = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['comb_field_display_type']  = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['chart_display_values']     = array();

           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['chart_combination_type']        = 'column2d';
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['chart_combination_order_rule']  = 'asc';
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['chart_combination_order_field'] = 'label';
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['chart_combination_order_rule_groupby']["data"]  = 'asc';
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['chart_combination_order_field_groupby']["data"] = 'label';
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['chart_combination_order_rule_groupby']["data_scgb_22"]  = 'asc';
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['chart_combination_order_field_groupby']["data_scgb_22"] = 'label';
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['chart_combination_order_rule_groupby']["data_scgb_23"]  = 'asc';
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['chart_combination_order_field_groupby']["data_scgb_23"] = 'label';
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['chart_combination_order_rule_groupby']["hora"]  = 'asc';
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['chart_combination_order_field_groupby']["hora"] = 'label';

           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_y_axys'][0] = 0;
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_y_axys'][1] = 1;
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_y_axys'][2] = 2;
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_y_axys'][3] = 3;

           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['SC_Gb_Free_cmp']["data"] = 'data';
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['SC_Gb_Free_cmp']["data_scgb_22"] = 'data_scgb_22';
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['SC_Gb_Free_cmp']["data_scgb_23"] = 'data_scgb_23';
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['comb_field_display_type'][2] = '';

           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['chart_display_values'][2] = null;


           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['summarizing_fields_order']['sc_free_group_by'][] = 2;
       }
      ...
      {
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['where_resumo']);
      }
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['res_hrz']))
      { 
          $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['res_hrz'] = $this->NM_totaliz_hrz;
      } 
      $this->NM_totaliz_hrz = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['res_hrz'];
      if (isset($_SESSION['scriptcase']['sc_aba_iframe']) && !$this->NM_export)
      {
          foreach ($_SESSION['scriptcase']['sc_aba_iframe'] as $aba => $apls_aba)
          {
              if (in_array("historico_nivel", $apls_aba))
              {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['iframe_menu'] && !$this->NM_export && (!isset($_SESSION['scriptcase']['menu_mobile']) || empty($_SESSION['scriptcase']['menu_mobile'])))
      {
          $this->aba_iframe = true;
      }
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['array_graf_pdf'] = array();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['opcao'] != "pdf")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['where_resumo'] = "";
      }
      $this->inicializa_vars();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['contr_array_resumo'] == "OK" && $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['contr_total_geral'] == "OK")
      {
          foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
          {
              $Arr_tot_name = "array_total_" . $cmp_gb;
              $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['arr_total'][$cmp_gb];
          }
      }
      else
      {
      ...
      $Sv_tot_ger = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['tot_geral'];
      $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['contr_total_geral'] = "N";
      $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['SC_Ind_Groupby'];
      $tp_tot = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['Res_search_metric_use']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['Res_search_metric_use'])) ? true : false;
      $this->Total->$Gb_geral($tp_tot);
      $this->array_total_geral = array();
      $this->array_total_geral[0] = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['tot_geral'][2];
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['tot_geral'][1]) || $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['tot_geral'][1] == 0)
      {
          $this->NM_res_sem_reg = true;
      }
      if (isset($Sv_tot_ger))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['tot_geral'] = $Sv_tot_ger;
          unset($Sv_tot_ger);
      }
      $this->resumo_init();
      ...
      $this->resumo_final();
      $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['contr_label_graf'] = array();
      if (isset($this->nmgp_label_quebras) && !empty($this->nmgp_label_quebras))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['contr_label_graf'] = $this->nmgp_label_quebras;
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['arr_total']['data'] = $this->array_total_data;
      $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['arr_total']['data_scgb_22'] = $this->array_total_data_scgb_22;
      $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['arr_total']['data_scgb_23'] = $this->array_total_data_scgb_23;
      $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['arr_total']['hora'] = $this->array_total_hora;
      $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['contr_array_resumo'] = "OK";
   }
   ...
   function completeMatrix()
   {
       ...
       //-----
       if ($this->NM_export && isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['json_use_label']) && !$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['json_use_label'])
       {
          $Lab_data = "data";
       ...
       $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['labels']['data'] = $Lab_data; 
       $prep_field['data_scgb_22'] = $Lab_data_scgb_22;
       $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['labels']['data_scgb_22'] = $Lab_data_scgb_22; 
       $prep_field['data_scgb_23'] = $Lab_data_scgb_23;
       $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['labels']['data_scgb_23'] = $Lab_data_scgb_23; 
       $prep_field['hora'] = $Lab_hora;
       $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['labels']['hora'] = $Lab_hora; 
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
       {
          $Str_gb .= ($Str_gb == "") ? "" : ",";
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['labels']['id']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['labels']['id'] = "Id"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['labels']['data']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['labels']['data'] = "Data"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['labels']['hora']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['labels']['hora'] = "Hora"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['labels']['nivel']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['labels']['nivel'] = "Nivel"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['labels']['volume']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['labels']['volume'] = "Volume"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['labels']['data_scgb_22']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['labels']['data_scgb_22'] = "Data"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['labels']['data_scgb_23']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['labels']['data_scgb_23'] = "Data"; 
       }
       //-----
       $ix = 0;
       $this->comp_field_nm = array();
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['summarizing_fields_order']['sc_free_group_by']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['summarizing_fields_order']['sc_free_group_by'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['summarizing_fields_order']['sc_free_group_by'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['summarizing_fields_display']['sc_free_group_by'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['summarizing_fields_display']['sc_free_group_by'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['summarizing_fields_display']['sc_free_group_by'] as $i_sum => $d_sum)
           {
      ...
      eval ($Str_gb);
       ...
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['SC_Gb_Free_cmp'] as $cmp_gb => $col_sql)
       {
       ...
       //-----
       $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_group_by'] = $this->comp_field;
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_x_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_x_axys'] = $this->comp_x_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_y_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_y_axys'] = $this->comp_y_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_fill']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_fill'] = $this->comp_fill;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order'] = $this->comp_order;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order_start']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order_start'] = $this->comp_order_start;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_tabular']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_tabular'] = $this->comp_tabular;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_tabular_hover']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_tabular_hover'] = $this->comp_tabular && $this->comp_tab_hover;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_tabular_seq']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_tabular_seq'] = $this->comp_tabular && $this->comp_tab_seq;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_tabular_label']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_tabular_label'] = $this->comp_tabular && $this->comp_tab_label;
       }

       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order_col']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order_col'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order_level']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order_level'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order_sort']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order_sort'] = '';
       }
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['ajax_nav'] && isset($_POST['parm']))
       { 
       ...
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order_sort'] = $_POST['sort_ord'];
           if ('' == $_POST['sort_ord'])
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order_col']  = 0;
           }
           ...
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order_col']  = $_POST['sort_col'];
           }
       }
       if (isset($_POST['change_sort']) && 'NEW' == $_POST['change_sort']) {
           for ($i = 0; $i < sizeof($this->comp_label); $i++) {
               if ($i == $_POST['sort_col']) {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order_start'][$i] = $_POST['sort_ord'];
               }
           }
       }

       $this->comp_x_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_x_axys'];
       $this->comp_y_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_y_axys'];
       $this->comp_fill        = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_fill'];
       $this->comp_order       = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order'];
       $this->comp_order_start = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order_start'];
       $this->comp_order_col   = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order_col'];
       $this->comp_order_level = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order_level'];
       $this->comp_order_sort  = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order_sort'];
       $this->comp_tabular     = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_tabular'];
       $this->comp_tab_hover   = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_tabular_hover'];
       $this->comp_tab_seq     = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_tabular_seq'];
       $this->comp_tab_label   = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_tabular_label'];
   ...
   function getLabelLink($param, $i_tmp = -1, $bProtect = true)
   {
       ...
       $Md5_Res    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@historico_nivel@SC_par@" . md5($Parms_Res);
       $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['LigR_Md5'][md5($Parms_Res)] = $Parms_Res;
       return $Md5_Res;
   }
   ...
   function makeTabular(&$a_data)
   {
       ...
       {
           $a_labels = array();
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['proc_pdf_vert'])
   {
     $this->comp_tab_hover = true;
   }
   ...
   function buildChart()
   {
       ...
       $prep_chart_config = array(
           'data' => array(
           '1' => array(
               'title'    => $this->getChartText(strip_tags("Nivel (%)")),
               'show_sub' => true,
               'subtitle' => "",
               'field_x'  => 'data',
               'field_y'  => $this->comp_sum_nm[2],
               'label_x'  => $this->getChartText(strip_tags($Lab_data)),
               'label_y'  => $this->getChartText(strip_tags("Nivel (%)")),
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['summarizing_fields_order']['sc_free_group_by'] as $i_summ_order)
       {
                   ...
                   foreach ($this->sc_comb_quick_index[$i_index_summ] as $i_index_comb_groupby)
                   {
                       ...
                       if (!isset($a_chart_combinations[$s_comb_index]))
                       {
                           $a_chart_combinations[$s_comb_index] = array(
                               ...
                               'label_x'     => '',
                               'label_y'     => '',
                               'xml_pholder' => sizeof($this->comp_y_axys) > $i_index_group + 1 ? 'sc_historico_nivel_' . session_id() . '_!!!C_' . ($i_index_group + 1) . '_' . $s_index_comb . ('' == $s_index_comb ? '' : '_') . '__SC_PLACEHOLDER__!!!.json' : '',
                               'xml'         => 'sc_historico_nivel_' . session_id() . '_!!!C_' . $i_index_group . '_' . $s_index_comb . '!!!.json',
                           );
                       }
       ...
       $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_charts']    = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_orig']      = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['comb_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['comb_table_def']  = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['anlt_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['anlt_table_def']  = array();
	...
	function filterChartsByGroupbyLevel() {
		...
		if (isset($nmgp_opcao) && ('pdf' == $nmgp_opcao || 'pdf_res' == $nmgp_opcao) && isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['chart_level']) && '' !== $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['chart_level']) {
			$applyFilter = true;
		}
		elseif (isset($nmgp_opcao) && 'image_res' == $nmgp_opcao && isset($_POST['chart_level']) && '' !== $_POST['chart_level']) {
			$applyFilter = true;
			$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['chart_level'] = $_POST['chart_level'];
		}
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_charts'] = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_orig'];

		if ($applyFilter) {
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_charts'] as $chartIndex => $chartData) {
				...
				if ($groupbyLevel > $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['chart_level']) {
					unset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_charts'][$chartIndex]);
				}
			}
		}

		$this->comp_chart_data = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_charts'];
	}
	...
	function filterChartsBySummarization() {
		...
		if (isset($nmgp_opcao) && ('pdf' == $nmgp_opcao || 'pdf_res' == $nmgp_opcao)) {
			...
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['summarizing_fields_display']['sc_free_group_by'] as $fieldIndex => $fieldData) {
	...
	function filterChartsByMultiseries() {
		...
		if ($applyFilter && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['summarizing_drill_down']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['summarizing_drill_down'])) {
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_charts'] as $chartIndex => $chartData) {
				$indexParts = explode('|', $chartIndex);
           ...
           foreach ($a_totals as $i_total => $values)
           {
               ...
               if (isset($this->comp_chart_config[$key_config]))
               {
                   ...
                   foreach ($a_keys as $tmp_key)
                   {
                       ...
                       $xml_links[]  = $this->getFusionLink('sc_historico_nivel_' . session_id() . '_!!!' . implode('_', array_merge(array($group + 1, $i_total), $link_index)) . '!!!.json');
                   }
                   ...
                   $this->comp_chart_data[$key_data]['xml']        = $this->getFusionLink('sc_historico_nivel_' . session_id() . '_!!!' . implode('_', array_merge(array($group, $i_total), $param)) . '!!!.json');
   ...
   function drawMatrix()
   {
       ...
       if ($this->NM_export)
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['arr_export']['label'] = $this->build_labels;
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['arr_export']['data']  = $this->build_data;
           return;
       }
      ...
      $TD_padding = (!$this->Print_All && $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['opcao'] == "pdf") ? " style=\"padding: 0px !important;\"" : "";
      ...
      if ($this->comp_tab_hover)
      {
       ...
       foreach ($this->build_data as $row_i => $lines)
       {
           ...
           foreach ($lines as $col_i => $columns)
           {
               ...
               if (0 <= $columns['level'])
               {
                   if ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['opcao'] != "pdf" && $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['embutida'])
                   {
           ...
           if (!$b_display)
           {
               if ($this->comp_tabular)
               {
                   ...
                   foreach ($this->comp_y_axys as $iYAxysIndex)
                   {
                       ...
                       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order_start'][$iYAxysIndex]))
                       {
                           $sInitialOrder   = '';
                           $sInitialDisplay = '; display: none';
                           $sInitialSrc     = '';
                       }
                       elseif ('asc' == $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_order_start'][$iYAxysIndex])
                       {
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['proc_pdf_vert']){ 
   ...
   function resumo_init()
   {
      ...
      $this->monta_html_ini();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['opcao'] != "pdf" && !$_SESSION['scriptcase']['proc_mobile'])
      {
   ...
   function monta_css()
   {
      global $nm_saida, $nmgp_tipo_pdf, $nmgp_cor_print;
       ...
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['embutida'])
       {
          if (($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
           { 
               if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css_bw']['historico_nivel']))
               {
                   $compl_css = str_replace(".", "_", $_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css_bw']['historico_nivel']) . "_";
               } 
           } 
           else 
           { 
               if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css']['historico_nivel']))
               {
                   $compl_css = str_replace(".", "_", $_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css']['historico_nivel']) . "_";
               } 
           }
       }
   ...
   function resumo_final()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['ajax_nav'])
      {
   ...
   {
      $this->Tot_ger = false;
      require_once($this->Ini->path_aplicacao . "historico_nivel_total.class.php"); 
      $this->Print_All = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['print_all'];
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['doc_word'] || $this->Ini->sc_export_ajax_img)
      { 
      ...
      $this->Total   = new historico_nivel_total($this->Ini->sc_page);
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['LigR_Md5'] = array();
   }
   ...
   function totaliza()
   {
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['arr_total'] as $cmp_gb => $resto)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['arr_total'][$cmp_gb];
      }
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['responsive_chart']['active']) {
$nm_saida->saida("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\"\r\n");
$nm_saida->saida("            \"http://www.w3.org/TR/1999/REC-html401-19991224/loose.dtd\">\r\n");
}
$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['dashboard_refresh_after_chart'] = 'resumo';
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['doc_word'])
{
      ...
      { 
           ...
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"historico_nivel_ajax.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\">\r\n");
           ...
           $nm_saida->saida(" </script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"historico_nivel_jquery.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"historico_nivel_message.js\"></script>\r\n");
...
$nm_saida->saida("   <link rel=\"stylesheet\" type=\"text/css\" href=\"" . $this->Ini->path_link . "historico_nivel/historico_nivel_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css\" />\r\n");
           $nm_saida->saida("   <link rel=\"stylesheet\" href=\"" . $this->Ini->path_prod . "/third/jquery_plugin/thickbox/thickbox.css\" type=\"text/css\" media=\"screen\" />\r\n");
           $nm_saida->saida("   <link rel=\"stylesheet\" type=\"text/css\" href=\"../_lib/buttons/" . $this->Ini->Str_btn_css . "\" /> \r\n");
      } 
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['num_css']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['num_css'] = rand(0, 1000);
      }
      $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_historico_nivel_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['num_css'] . '.css', 'w');
      if (($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
      {
      ...
      @fclose($NM_css);
     $this->Ini->summary_css = $this->Ini->path_imag_temp . '/sc_css_historico_nivel_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['num_css'] . '.css';
     if ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['opcao'] == "print")
     {
         $nm_saida->saida("  <style type=\"text/css\">\r\n");
         $NM_css = file($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_historico_nivel_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['num_css'] . '.css');
...
$nm_saida->saida("      url: \"historico_nivel_save_grid.php\",\r\n");
...
if ($_SESSION['scriptcase']['proc_mobile'])
{
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['responsive_chart']['active']) {
          ...
          $remove_margin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['dashboard_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['dashboard_info']['remove_margin'] ? 'margin: 0;' : '';
          $remove_border = isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['dashboard_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['dashboard_info']['remove_border'] ? 'border-width: 0;' : '';
          $vertical_center = '';
      $nm_saida->saida(" <BODY class=\"" . $this->css_scGridPage . "\" style=\"" . $remove_margin . $vertical_center . "\">\r\n");
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['embutida'])
      {
...
;
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['embutida'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['doc_word'])
      { 
          $nm_saida->saida("      <STYLE>\r\n");
   ...
   function monta_barra_top_normal()
   {
      ...
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['ajax_nav'] && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['where_resumo_search']))
      {
          $this->NM_res_sem_reg = false;
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['embutida'])
      ...
      {
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bexcel", "", "", "Rxls_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "historico_nivel/historico_nivel_config_xls.php?script_case_init=" . $this->Ini->sc_page . "&app_name=historico_nivel&nm_tp_xls=xlsx&nm_tot_xls=S&nm_tot_xls=S&nm_res_cons=s&nm_ini_xls_res=chart&nm_all_modules=chart&password=n&summary_export_columns=N&origem=chart&language=&KeepThis=true&TB_iframe=true&modal=true", "", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
      }
      if (!$this->grid_emb_form && $this->nmgp_botoes['pdf'] == "on" && !$this->NM_res_sem_reg)
      {
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bpdf", "", "", "Rpdf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "historico_nivel/historico_nivel_config_pdf.php?nm_opc=pdf&nm_target=0&nm_cor=cor&papel=8&orientacao=1&bookmarks=1&largura=1200&conf_larg=S&conf_fonte=10&grafico=S&sc_ver_93=" . s . "&nm_tem_gb=s&nm_res_cons=s&nm_ini_pdf_res=chart&nm_all_modules=chart&password=n&summary_export_columns=N&pdf_zip=N&origem=chart&language=pt_br&conf_socor=N&is_chart_app=Y&nm_label_group=N&nm_all_cab=S&nm_all_label=S&nm_orient_grid=2&script_case_init=" . $this->Ini->sc_page . "&app_name=historico_nivel&KeepThis=true&TB_iframe=true&modal=true", "", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
      }
      if (is_file($this->Ini->path_css . "schemas.ini") && !$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['opc_psq'] && !$this->grid_emb_form && !$this->NM_res_sem_reg)
      {
         ...
         if (is_file("historico_nivel_help.txt") && !$this->grid_emb_form && !$this->NM_res_sem_reg)
         {
            $Arq_WebHelp = file("historico_nivel_help.txt"); 
   ...
   function monta_html_fim()
   {
      ...
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['historico_nivel']['glo_nm_path_prod'] . "/third/sweetalert/sweetalert2.all.min.js\"></script>\r\n");
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['historico_nivel']['glo_nm_path_prod'] . "/third/sweetalert/polyfill.min.js\"></script>\r\n");
...
$nm_saida->saida("       scJs_alert('" . $mensagem . "', $jsonParams);\r\n");
      ...
      elseif ($this->Ini->Export_html_zip || ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['opcao'] == "pdf" && $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['graf_pdf'] == "S"))
      {
          $this->grafico_pdf();
      }
      else
...
{ 
      ...
      $nm_saida->saida("</TABLE>\r\n");
       ...
       $nm_saida->saida("  tb_show('', 'historico_nivel_config_pivot.php?nome_apl=historico_nivel&sc_page=" . NM_encode_input($this->Ini->sc_page) . "&language=pt_br&TB_iframe=true&modal=true&height=300&width=500', '');\r\n");
       $nm_saida->saida("}\r\n");
   ...
   $nm_saida->saida("   </form> \r\n");
          ...
          $nm_saida->saida("               nm_submit_modal(\"" . $this->Ini->path_link . "historico_nivel/historico_nivel_export_email.php?script_case_init={$this->Ini->sc_page}&path_img={$this->Ini->path_img_global}&path_btn={$this->Ini->path_botoes}&sType=\" + str_type +\"&sAdd=__E__SC_module_export=\" + SC_module_export + \"__E__nmgp_tp_xls=\" + tp_xls + \"__E__nmgp_tot_xls=\" + tot_xls + \"__E__nmgp_password=\" + password + \"&KeepThis=true&TB_iframe=true&modal=true\", bol_param);\r\n");
          $nm_saida->saida("       }\r\n");
          $nm_saida->saida("       else\r\n");
          $nm_saida->saida("       {\r\n");
          ...
          $nm_saida->saida("       document.Fexport.action = \"historico_nivel_export_ctrl.php\";\r\n");
      ...
      if (!extension_loaded("zip"))
      {
          ...
          $nm_saida->saida("       document.F3.SC_module_export.value = z;\r\n");
          $nm_saida->saida("  }\r\n");
      $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['historico_nivel_iframe_params'] = array(
      ...
      $Md5_pdf    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@historico_nivel@SC_par@" . md5($prep_parm_pdf);
      $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['Md5_pdf'][md5($prep_parm_pdf)] = $prep_parm_pdf;
...
;
   ...
   function monta_html_ini_pdf()
   {
      global $nm_saida;
       $tp_quebra = "";
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['num_css']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['num_css']))
       {
           $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_historico_nivel_grid_' . $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['num_css'] . '.css', 'a');
           $NM_css_file = $this->Ini->root . $this->Ini->path_link . "historico_nivel/historico_nivel_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']). ".css";
       ...
       if ($this->Print_All || $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['print_all'])
       {
       $tp_quebra = "<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>";
       }
...
//--- 
 function grafico_pdf()
 {
   ...
   $this->Graf->Ini    = $this->Ini;
   $this->Graf->Lookup = $this->Lookup;
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_charts']) && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['skip_charts']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['skip_charts']))
   {
 ...
 { 
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_charts'] as $chart_index => $chart_data)
       {
 ...
 { 
           ...
           if ($this->progress_fp)
           {
               historico_nivel_pdf_progress_call($this->progress_tot . "_#NM#_" . $this->progress_now . "_#NM#_" . $sChartLang . " " . $iChartCount . "...\n", $this->Ini->Nm_lang, true);
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['this_chart_label'] = '';
           $this->Graf->monta_grafico($chart_index, 'pdf');
           if ('' != $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['this_chart_label']) {
               $nm_saida->saida("<script type=\"text/javascript\">$(\"#$random_id\").html(\"{$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['this_chart_label']}\")</script>\r\n");
           }
       ...
       if ($bChartFP)
       {
           ...
           historico_nivel_pdf_progress_call(100 . "_#NM#_" . 90 . "_#NM#_" . $lang_protect . "...\n", $this->Ini->Nm_lang);
           fwrite($this->progress_fp, 90 . "_#NM#_" . $lang_protect . "...\n");
           fclose($this->progress_fp);
       }
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['charts_html']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['charts_html'])
       {
            $nm_saida->saida("<script type=\"text/javascript\">\r\n");
            $nm_saida->saida("{$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['charts_html']}\r\n");
            $nm_saida->saida("</script>\r\n");
       }
   }
       $nm_saida->saida("</body>\r\n");
...
//--- 
 ...
 {
   global $nm_saida, $nm_lang;
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['chart_list']))
   {
       $prim_graf = true;
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['chart_list'] as $arr_chart)
       {
           if (!$prim_graf)
           {
               $nm_saida->saida("<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>\r\n");
...
//--- 
 function grafico_bot()
 {
   ...
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_charts']))
   {
       $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['graf_bot'] = true;
       $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['graf_first'] = true;
       require_once($this->Ini->path_aplicacao . $this->Ini->Apl_grafico); 
       $this->Graf  = new historico_nivel_grafico();
       $this->Graf->Db     = $this->Db;
       $this->Graf->Erro   = $this->Erro;
       ...
       $nm_saida->saida("<TR>\r\n");
       $iChartCount = 1;
       $iChartTotal = sizeof($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_charts']);
       $sChartLang  = isset($this->Ini->Nm_lang['lang_pdff_pcht']) ? $this->Ini->Nm_lang['lang_pdff_pcht'] : 'Generating chart';
       if (!NM_is_utf8($sChartLang))
       {
           $sChartLang = sc_convert_encoding($sChartLang, "UTF-8", $_SESSION['scriptcase']['charset']);
      ...
      if ($bPDF && (!isset($this->progress_fp) || !$this->progress_fp))
      {
           ...
           historico_nivel_pdf_progress_call("PDF\n", $this->Ini->Nm_lang);
           historico_nivel_pdf_progress_call("\n", $this->Ini->Nm_lang);
           historico_nivel_pdf_progress_call("\n", $this->Ini->Nm_lang);
           historico_nivel_pdf_progress_call("100\n", $this->Ini->Nm_lang);
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['pivot_charts'] as $chart_index => $chart_data)
       {
           ...
           if ($bPDF)
           {
               ...
               if ($this->progress_fp)
               {
                   historico_nivel_pdf_progress_call($this->progress_tot . "_#NM#_" . $this->progress_now . "_#NM#_" . $sChartLang . " " . $iChartCount . "/" . $iChartTotal . "...\n", $this->Ini->Nm_lang, true);
       ...
       $nm_saida->saida("</TD>\r\n");
       $nm_saida->saida("</TR>\r\n");
       $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['graf_bot'] = false;
       if ($bPDF && isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['charts_html']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['charts_html'])
       {
           $nm_saida->saida("<script type=\"text/javascript\">\r\n");
           $nm_saida->saida("{$_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['charts_html']}\r\n");
           $nm_saida->saida("</script>\r\n");
...
//--- 
   function monta_resumo_search()
   {
      ...
      $this->html_resumo_search();
       ...
       { 
           ...
           $_SESSION['scriptcase']['saida_html'] = "";
           if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']))
           { 
               $this->Ini->Arr_result['setArr'][] = array('var' => 'Tab_obj_res_combo', 'value' => '');
               foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look'] as $cada_combo => $opcs_combo)
               { 
               ...
               if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']))
               { 
   ...
   {
       global $nm_saida;
       $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['res_search_label'] = array();
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search']         = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']    = array();
   ...
   {
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search']['data_SC_1']))
       {
       ...
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_1']))
       {
           $result_look = $this->Lookup->lookup_res_data_SC_1();
           foreach ($result_look['set_option'] as $ind => $opcs)
           {
               $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_1']) && in_array($opcs['opt'], $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_1'])) ? " selected" : "";
   ...
   {
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search']['data_SC_4']))
       {
           $lin_obj .= "   <div class='scGridSummarySearchCampo active' style='float:left;'>";
       }
       else
       ...
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4']) && in_array('month_jan', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"month_jan\"" . $selected . ">" . $this->Ini->Nm_lang['lang_mnth_janu'] . "</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4']) && in_array('month_feb', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"month_feb\"" . $selected . ">" . $this->Ini->Nm_lang['lang_mnth_febr'] . "</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4']) && in_array('month_mar', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"month_mar\"" . $selected . ">" . $this->Ini->Nm_lang['lang_mnth_marc'] . "</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4']) && in_array('month_apr', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"month_apr\"" . $selected . ">" . $this->Ini->Nm_lang['lang_mnth_apri'] . "</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4']) && in_array('month_may', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"month_may\"" . $selected . ">" . $this->Ini->Nm_lang['lang_mnth_mayy'] . "</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4']) && in_array('month_jun', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"month_jun\"" . $selected . ">" . $this->Ini->Nm_lang['lang_mnth_june'] . "</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4']) && in_array('month_jul', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"month_jul\"" . $selected . ">" . $this->Ini->Nm_lang['lang_mnth_july'] . "</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4']) && in_array('month_aug', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"month_aug\"" . $selected . ">" . $this->Ini->Nm_lang['lang_mnth_augu'] . "</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4']) && in_array('month_sep', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"month_sep\"" . $selected . ">" . $this->Ini->Nm_lang['lang_mnth_sept'] . "</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4']) && in_array('month_oct', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"month_oct\"" . $selected . ">" . $this->Ini->Nm_lang['lang_mnth_octo'] . "</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4']) && in_array('month_nov', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"month_nov\"" . $selected . ">" . $this->Ini->Nm_lang['lang_mnth_nove'] . "</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4']) && in_array('month_dec', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_4'])) ? " selected" : "";
   ...
   {
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search']['data_SC_5']))
       {
           $lin_obj .= "   <div class='scGridSummarySearchCampo active' style='float:left;'>";
       }
       else
       ...
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day1', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day1\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "1</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day2', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day2\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "2</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day3', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day3\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "3</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day4', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day4\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "4</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day5', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day5\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "5</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day6', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day6\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "6</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day7', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day7\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "7</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day8', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day8\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "8</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day9', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day9\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "9</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day10', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day10\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "10</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day11', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day11\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "11</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day12', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day12\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "12</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day13', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day13\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "13</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day14', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day14\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "14</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day15', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day15\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "15</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day16', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day16\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "16</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day17', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day17\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "17</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day18', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day18\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "18</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day19', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day19\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "19</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day20', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day20\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "20</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day21', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day21\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "21</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day22', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day22\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "22</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day23', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day23\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "23</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day24', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day24\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "24</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day25', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day25\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "25</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day26', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day26\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "26</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day27', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day27\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "27</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day28', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day28\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "28</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day29', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day29\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "29</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day30', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
       $lin_obj  .= "   <option value=\"day30\"" . $selected . ">" . $this->Ini->Nm_lang['lang_srch_days'] . "30</option>";
       $selected  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5']) && in_array('day31', $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['resumo_search_look']['data_SC_5'])) ? " selected" : "";
   ...
   //---- 
   function adiciona_registro($max_nivel, $quebra_data, $quebra_data_scgb_22, $quebra_data_scgb_23, $quebra_hora, $quebra_data_orig, $quebra_data_scgb_22_orig, $quebra_data_scgb_23_orig, $quebra_hora_orig)
   {
      $contr_arr = "";
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
      {
          $Name_orig  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['SC_Gb_Free_orig'][$cmp_gb])) ? $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['SC_Gb_Free_orig'][$cmp_gb] : $cmp_gb;
          ...
          $cmp_look   = "Cmp_formt";
          $cmp_orig   = "Cmp_arg";
          if (empty($max_nivel)) {$max_nivel = '0';}
          eval ('
          if (!isset($this->' . $arr_name . '))
          {
              $this->' . $arr_name . '[0] = 1;
              $this->' . $arr_name . '[1] = ' . $max_nivel . ';
          ...
          {
              $this->' . $arr_name . '[0]++;
              $this->' . $arr_name . '[1] = max($this->' . $arr_name . '[1], ' . $max_nivel . ');
          }
          ');
      }
   }
...
//
   function nm_acumula_resumo($nm_tipo="resumo")
   {
     ...
     $nmgp_select .= " " . $_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['where_pesq']; 
     $campos_order = "";
     foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_nivel']['SC_Gb_Free_sql'] as $cmp_var => $resto)
     {
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->nivel, $this->data, $this->data_scgb_22, $this->data_scgb_23, $this->hora, $this->data_orig,  $this->data_scgb_22_orig,  $this->data_scgb_23_orig,  $this->hora_orig);
            }
   }
//
