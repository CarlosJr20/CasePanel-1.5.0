...
class historico_volume_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['historico_volume']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['historico_volume']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['tot_geral'] = array() ;  
    if ($res_limit && isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['sql_tot_res']))
    {
        $nm_comando = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['sql_tot_res'];
    }
    else
    {
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), max(volume), varp(volume) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*), max(volume), variance(volume) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['where_pesq']; 
      } 
    }
      ...
      {
          $vl_statistic = $this->Calc_statist_manual_sc_free_group_by($_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['where_pesq']);
      ...
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_ibase) && $this->Ini->Ibase_version == "old")
      {
          $vl_statistic = $this->Calc_statist_manual_sc_free_group_by($_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['where_pesq']);
          foreach ($vl_statistic as $ind => $val)
          {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['tot_geral'][3] = $rt->fields[2]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['contr_total_geral'] = "OK";
   } 


   function Calc_statist_manual_sc_free_group_by($sql_where="")
   {
       $comando  = "select volume from " . $this->Ini->nm_tabela . "  " .  $sql_where;
   ...
   function quebra_data_sc_free_group_by($data, $Where_qb, $Cmp_Name) 
   {
      ...
      $nm_comando = $this->Ajust_statistic($nm_comando);
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['where_pesq'])) 
      { 
      ...
      { 
         $where_ok = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['where_pesq'] . " and " . $Where_qb ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
   ...
   //-----  data_scgb_22
   function quebra_data_scgb_22_sc_free_group_by($data_scgb_22, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_data_scgb_22 = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), max(volume), varp(volume) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['where_pesq']; 
      } 
      ...
      $tot_data_scgb_22[0] = NM_encode_input(sc_strip_script($data_scgb_22)) ; 
      $tot_data_scgb_22[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_data_scgb_22[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_data_scgb_22[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_data_scgb_22;
   } 

   //-----  data_scgb_23
   function quebra_data_scgb_23_sc_free_group_by($data_scgb_23, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_data_scgb_23 = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), max(volume), varp(volume) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['where_pesq']; 
      } 
      ...
      $tot_data_scgb_23[0] = NM_encode_input(sc_strip_script($data_scgb_23)) ; 
      $tot_data_scgb_23[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_data_scgb_23[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_data_scgb_23[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_data_scgb_23;
   } 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("pt_br");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('data' => "data",'data_scgb_22' => "data",'data_scgb_23' => "data",'hora' => "hora");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      $cmp_sql_tp_num = array('id' => 'N','nivel' => 'N','volume' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), max(volume), varp(volume)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         $comando  = "select count(*), max(SC_metric1), varp(SC_metric1)#@#cmps_quebras#@# from (";
         $comando .= "select volume as SC_metric1, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
      ...
      { 
         $cmd_simp = "select count(*), max(volume), variance(volume)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         $comando  = "select count(*), max(SC_metric1), variance(SC_metric1)#@#cmps_quebras#@# from (";
         $comando .= "select volume as SC_metric1, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
         $comando .= "select " . $cmps_quebras2 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
         $comando .= " ON " . $join;
      } 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['historico_volume']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['data_scgb_22']['reg'] = "N";
      $format_dimensions['data_scgb_22']['msk'] = "F";
      $format_dimensions['data_scgb_23']['reg'] = "S";
      $format_dimensions['data_scgb_23']['msk'] = "";
