...
class chart_historico_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['chart_historico']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['chart_historico']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(nivel), sum(volume) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*), sum(nivel), sum(volume) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['tot_geral'][3] = $rt->fields[2]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['contr_total_geral'] = "OK";
   } 
   ...
   function quebra_data_sc_free_group_by($data, $Where_qb, $Cmp_Name) 
   {
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['where_pesq'])) 
      { 
   ...
   //-----  data_scgb_14
   function quebra_data_scgb_14_sc_free_group_by($data_scgb_14, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_data_scgb_14 = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(nivel), sum(volume) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['where_pesq']; 
      } 
      ...
      $tot_data_scgb_14[0] = NM_encode_input(sc_strip_script($data_scgb_14)) ; 
      $tot_data_scgb_14[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_data_scgb_14[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_data_scgb_14[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_data_scgb_14;
   } 

   //-----  data_scgb_15
   function quebra_data_scgb_15_sc_free_group_by($data_scgb_15, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_data_scgb_15 = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(nivel), sum(volume) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['where_pesq']; 
      } 
      ...
      $tot_data_scgb_15[0] = NM_encode_input(sc_strip_script($data_scgb_15)) ; 
      $tot_data_scgb_15[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_data_scgb_15[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_data_scgb_15[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_data_scgb_15;
   } 

   //-----  data_scgb_16
   function quebra_data_scgb_16_sc_free_group_by($data_scgb_16, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_data_scgb_16 = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(nivel), sum(volume) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['where_pesq']; 
      } 
      ...
      $tot_data_scgb_16[0] = NM_encode_input(sc_strip_script($data_scgb_16)) ; 
      $tot_data_scgb_16[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_data_scgb_16[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_data_scgb_16[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_data_scgb_16;
   } 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("pt_br");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('data' => "data",'data_scgb_14' => "data",'data_scgb_15' => "data",'data_scgb_16' => "data",'hora' => "hora");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(nivel), sum(volume)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1), sum(SC_metric2)#@#cmps_quebras#@# from (";
         $comando .= "select nivel as SC_metric1,volume as SC_metric2, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_historico']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['data_scgb_14']['reg'] = "S";
      $format_dimensions['data_scgb_14']['msk'] = "";
      $format_dimensions['data_scgb_15']['reg'] = "N";
      $format_dimensions['data_scgb_15']['msk'] = "F";
      $format_dimensions['data_scgb_16']['reg'] = "S";
      $format_dimensions['data_scgb_16']['msk'] = "";
